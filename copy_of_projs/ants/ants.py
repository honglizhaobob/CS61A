...
class Place(object):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                if self.ant.can_contain(insect):
                    # 1-- if the ant currently in this place can contain the insect being added
                    self.ant.contain_ant(insect)  # we contain this insect
                    insect.place = self
                elif insect.can_contain(self.ant):
                    # 2-- vice versa to 1, if the insect being added can contain the ant currently in this place
                    insect.contain_ant(self.ant)  # we contain this ant
                    self.ant = insect
                    insect.place = self
                else:
                    # 3 -- neither can do, raise assertion
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 13
            "*** YOUR CODE HERE ***"
            if isinstance(insect, QueenAnt) and not insect.is_fake:  # if insect about to be removed is an instance of the QueenAnt class, and that QueenAnt is NOT fake
                return  # do nothing
...
class Insect(object):
    ...
    is_watersafe = False  # added(P11)-- Most insects are not watersafe
...
#-----------added instance attributes for EC, if error persists, discard completely
...
#-----------
    ...
    def blocked(self):
        ...
        # BEGIN Problem 7
        if (self.place.ant is not None) and (self.place.ant.blocks_path == True):  # -- bee is blocked if there is an ant, and the ant blocks_path
            return True
        return False
        # END Problem 7
...
class Ant(Insect):
    ...
    food_cost = 0
    # ADD CLASS ATTRIBUTES HERE
    blocks_path = True  # added -- most ants block path
    is_container = False  # added--most ants are not container
    buffed = False  # added -- Q13: queen doubles damage
...
class ThrowerAnt(Ant):
    ...
    food_cost = 3
    min_range = 0
    max_range = float('inf')  # want a huge number to reflect that ThrowerAnt has no max range
    ...
    def nearest_bee(self, hive):
        ...
        entrance_transition = 0  # track range
        search_place = self.place  # we start by searching for bees beginning from our current place
        while search_place is not hive:
            if (search_place.bees) and (self.min_range < entrance_transition <= self.max_range):
                self.place.bees = search_place.bees
                break  # avoid infinite loop
            else:  # if didn't pass range check, move on
                entrance_transition = entrance_transition + 1
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        # we test if armor is going to be <= 0
        # if FireAnt is going to die, we trigger the last strike feature
        if self.armor - amount <= 0:
            for bee in list(self.place.bees):
                bee.reduce_armor(self.damage)
        # if not, it is just the regular reduce_armor
...
class HungryAnt(Ant):
    ...
    # OVERRIDE CLASS ATTRIBUTES HERE
    food_cost = 4
    time_to_digest = 3  # added -- 3 turns to digest
    # BEGIN Problem 6
    implemented = True   # Change to True to view in the GUI
    ...
    def __init__(self, armor=1):
        ...
        "*** YOUR CODE HERE ***"
        Insect.__init__(self, armor)
        self.digesting = 0  # added inst attr -- counts the # of turns left to digest
        # END Problem 6

    def eat_bee(self, bee):
        # BEGIN Problem 6
        "*** YOUR CODE HERE ***"
        # in this function, we need to implement HungryAnt's attack method - reduce bee's armor directly to 0
        if bee:
            bee.reduce_armor(bee.armor)  # -- eaten!
            self.digesting = self.time_to_digest  # -- reset timer, notice that instances can have different digesting time
    ...
    def action(self, colony):
        # BEGIN Problem 6
        "*** YOUR CODE HERE ***"
        if self.digesting == 0:  # -- if not digesting, eat a randomly picked bee
...
class NinjaAnt(Ant):
    ...
    # OVERRIDE CLASS ATTRIBUTES HERE
    food_cost = 5
    blocks_path = False  # overrid -- NinjaAnts do not block path
    # BEGIN Problem 7
    implemented = True   # Change to True to view in the GUI
    # END Problem 7
    ...
    def action(self, colony):
        # BEGIN Problem 7
        "*** YOUR CODE HERE ***"
        if self.place.bees:  # -- if there are bees, we damage them
            for bee in list(self.place.bees):
                bee.reduce_armor(self.damage)
        # END Problem 7
...
class BodyguardAnt(Ant):
    ...
    def contain_ant(self, ant):
        # BEGIN Problem 9
        "*** YOUR CODE HERE ***"
        self.contained_ant = ant  # assign ant to container
        # END Problem 9
    ...
    def action(self, colony):
        ...
        if self.contained_ant is not None:
            self.contained_ant.action(colony)  # if there's an ant in container, container performs contained_ant's action
        # END Problem 9
...
class TankAnt(BodyguardAnt):
    ...
    def action(self, colony):
        # BEGIN Problem 10
        "*** YOUR CODE HERE ***"
        for bee in list(self.place.bees):  # again, need to create copy of list to avoid skipping some bees
            bee.reduce_armor(self.damage)
        # -- first we deal the damage to all bees in its place
        BodyguardAnt.action(self, colony)  # then simply call super class method to inherit
        # END Problem 10
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        # BEGIN Problem 11
        "*** YOUR CODE HERE ***"
        Place.add_insect(self, insect)  # add insect first regardless
        # if insect is not watersafe, reduce armor to 0
...
class QueenAnt(ScubaThrower):  # You should change this line -- Changed P13
    # END Problem 13
    ...
    # BEGIN Problem 13
    implemented = True   # Change to True to view in the GUI
    is_imposter = False  # added-- switch to True for all subsequent insts
    # END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        "*** YOUR CODE HERE ***"
        Insect.__init__(self, armor)
        self.is_fake = QueenAnt.is_imposter  # very first inst should not be impostor
    ...
    def action(self, colony):
        ...
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        double_ants = []  # store the ants
#----- this block keeps track of all ants behind the queen

        def list_infantry(place):  # should call this on self.place
            # starting from self.place, check all ants behind and store them into the list
            curr_place = place
            while curr_place.exit is not None:  # --traverse through all places behind
                if not isinstance(curr_place.ant, QueenAnt) and curr_place.ant is not None:  # only add if ant is not queen
                    if curr_place.ant.is_container and curr_place.ant.contained_ant is not None:
                        double_ants.extend([curr_place.ant.contained_ant])
                    double_ants.extend([curr_place.ant])
...
#-----
        ...
        if self.is_fake:
            ...
            #-- behaves just the same as a ThrowerAnt
            ThrowerAnt.action(self, colony)
        #-- additionally, doubles damage
            list_infantry(self.place)  # alter double_ants
            for ant in double_ants:
                if ant.buffed is False:
                    ant.damage *= 2  # double damage of ant in the list
                    ant.buffed = True
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        # need to make sure that this is the TRUE queen ant
        if not self.is_fake:
            # if this queen is real, reduce armor normally
            Insect.reduce_armor(self, amount)  # notice the other part should be in the action(), not here
        if self.armor <= 0:
            bees_win()  # after reducing, if subzero, bees_win()
        # END Problem 13
...
def make_scare(action, bee):
    ...
    # BEGIN Problem EC
    "*** YOUR CODE HERE ***"
    # if scared_already, do nothing
    if bee.scared_already:
        return action
    def scared_action(colony):
        # Assign attributes
        bee.scared_already = True
        bee.direction = -1
        if bee.place is not colony.hive:
            bee.destination = bee.place.entrance
        else:
            bee.destination = bee.place
        action(colony)
        bee.direction = 1
...
def apply_effect(effect, bee, duration):
    ...
    track_duration = 0
    def take_effect(colony):
        nonlocal track_duration
        if track_duration < duration:
            new_action(colony)
            track_duration += 1
        else:
            original_action(colony)
    bee.action = take_effect
    # END Problem EC
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        if target:
            apply_effect(make_slow, target, 3)  # target remains slowed for 3 rounds
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        "*** YOUR CODE HERE ***"
        if target:
            apply_effect(make_scare, target, 2)  # target remains scared for 2 rounds
        # END Problem EC
