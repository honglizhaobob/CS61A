...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # if is operator
        # use a lambda function to eval all items in expr
        if isinstance(operator, MacroProcedure):
            new_expr = MacroProcedure.apply_macro(operator, expr.second, env)
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('Number of values does not match number of formals')
        for _ in range(len(formals)):  # repeat binding
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        if self.use_env is True:
            python_args.append(env)  # if use environment, add to the end of the list
        try:
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        env.define(expressions.first, scheme_eval(expressions.second.first, env))  # now exists in env; eval operands first, then bind result
        return expressions.first
        # END PROBLEM 6
    elif isinstance(target, Pair) and scheme_symbolp(target.first):
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
       # env.define (target.first, MacroProcedure(target.sedond, expr.second, env))
        # return target.first

        func_name = target.first
        formals = target.second  # x,y,z ... etc, parameters
...
def make_let_frame(bindings, env):
    ...
    formals, vals = nil, nil  # initialize
    while len(bindings):
        curr = bindings.first  # (x 2) .. (y 3)
        check_form(curr, 2, 2)  # check each binding should be a pair
        formals = Pair(curr.first, formals)  # Pair(x, ()) --> Pair(x, Pair(y, ()))
        vals = Pair(scheme_eval(curr.second.first, env), vals)
        bindings = bindings.second
    check_formals(formals)
    return env.make_child_frame(formals, vals)
    #let_frame = env.make_child_frame()
    # END PROBLEM 15
